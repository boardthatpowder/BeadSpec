...
The app SHALL expose a "Changes" view accessible from the top navigation bar alongside the existing list/detail views. The view reads `openspec/changes/` in the current project root (resolved from `AppState::current_project()`). When no project is open, the view shows an empty state.
