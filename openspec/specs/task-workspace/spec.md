...
Defines the IDE-style tabbed workspace that hosts `TaskDetailPanel` instances in a tree of resizable panes. Covers preview vs. pinned tab semantics, tab management, split panes, recently-closed stack, drag-to-reorder, keyboard shortcuts, workspace persistence, and the empty-pane placeholder. This capability replaces the single-task detail pane with a multi-tab, multi-pane working environment.
