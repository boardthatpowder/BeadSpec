...
Defines the unified Settings dialog that consolidates all user preferences, feature toggles, binary path overrides, and deployment-varying configuration into a single surface backed by `settings.json` via `@tauri-apps/plugin-store`.
