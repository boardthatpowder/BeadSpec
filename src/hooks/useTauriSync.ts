...
export function useTauriSync(activeProject?: string) {
  ...
  useEffect(() => {
    ...
    const unlistenChanged = listen<TasksChangedPayload>('tasks_changed', event => {
      ...
      queryClient.invalidateQueries({ queryKey: ['tasks', project], exact: false })
    })

    // Listen for full list refresh (structural changes)
    const unlistenListChanged = listen<TaskListChangedPayload>('task_list_changed', event => {
    ...
    const unlistenBdNotFound = listen('bd_not_found', () => {
      console.warn('[Beads UI] bd CLI not found — write operations will fail')
