...
export function TrayPopover() {
  ...
  const [form, setForm] = useState<QuickCreateState>({ title: '', priority: 2 })
  ...
  const createTask = async () => {
    ...
    setCreating(true)
    try {
      const result = await unwrap(commands.createTask(project, form.title.trim(), null, form.priority, null))
  ...
  return (
    <div className="fixed bottom-16 right-4 z-50 w-72 bg-neutral-900 border border-neutral-700 rounded-xl shadow-2xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <IconButton label="Close" onClick={() => setOpen(false)} className="text-neutral-500 hover:text-neutral-300 text-xs">✕</IconButton>
      </div>

      {openCount !== null && (
        <div className="mb-3 text-sm text-neutral-400">
          <span className="text-white font-semibold">{openCount}</span> open tasks
        </div>
      )}
      ...
      <button
        onClick={() => { commands.focusMainWindow(); setOpen(false) }}
